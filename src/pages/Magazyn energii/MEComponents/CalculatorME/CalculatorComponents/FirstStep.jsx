...
const FirstStep = ({
  ...
  return (
    <>
      <div className="grid gap-5 mt-10 md:grid-cols-2 md:gap-20 text-ocean-green-800">
        <div className="flex flex-col gap-5 ">
          <h4 className="text-2xl font-bold">Dotychczasowy rachunek</h4>
          <p className="font-medium ">
            Wybierz, za jaki okres podasz wysokość opłat za prąd
          </p>
          <div className="flex flex-col flex-wrap gap-3 md:flex-row justify-stretch">
            <button
        ...
        <div className="">
          <h4 className="mb-2 text-xl font-medium">
            Wpisz wysokość {year ? "rocznych" : ""}
            {twoMonth ? "2-miesięcznych" : ""}
            {oneMonth ? "miesięcznych" : ""} opłat
          </h4>
          <div>
            <div className="w-72">
          ...
          <div className="p-5 mt-5 rounded  shadow-md md:mt-20 bg-gradient-to-br from-ocean-green-100 to-ocean-green-200 shadow-ocean-green-200">
            <p className="">
              ...
              wynosi około {calculation} kWh.
            </p>
            <p>
              Przyjęta stawka za 1 kWh: {priceForkWp} zł
              <button
                onClick={() => setShowPriceForkWp(!showPriceForkWp)}
                className="pl-2 font-bold"
              >
                {showPriceForkWp ? "Zwiń" : "Rozwiń"}
              </button>
            </p>
            {showPriceForkWp ? (
              <ChangePrice price={priceForkWp} setPrice={setPriceForkWp} />
            ) : (
              ""
            )}
          </div>
